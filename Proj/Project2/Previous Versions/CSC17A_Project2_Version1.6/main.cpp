 ...
 * Version 1.6:
 * Altered the way the number of games played is stored, so the correct amount 
 * of players' info will be output at the end of the game. Used a class with a 
 * derived class that stored the number of quit games, which included an overridden 
 * function.
 */
...
//Function Prototypes
void gmeInfo();
UsrInfo game(fstream &, fstream &, QuitGIn);
UsrInfo svdgame(fstream &, fstream &, QuitGIn);
...
int main(int argc, char** argv) {
    ...
    QuitGIn quit; //Stores number of quit games
    ...
    do {
            ...
            switch (choice) {
                    ...
                    case '2': {player=game(binfile, savload, quit); numg++;}; break;
                    case '3': {player=svdgame(binfile, savload, quit); numg++;}; break;
